...
def parse_args():
    parser = argparse.ArgumentParser(description='LASSO Regression model')
    parser.add_argument('--input', type=str, required=True, help='Input CSV file path')
    parser.add_argument('--output', type=str, required=True, help='Output CSV file path')
    parser.add_argument('--alpha', type=float, default=1.0, help='LASSO regularization parameter')
...
def main():
    ...
    if 'score' in df.columns:
        ...
        if args.auto_tune:
            ...
            grid_search = GridSearchCV(Lasso(random_state=42), param_grid, cv=3, scoring='r2')
        ...
        # 保存特征重要性
        feature_importance.to_csv('lasso_feature_importance.csv', index=False)
        
        print(f"\n预测结果已保存到 {args.output}")
        print("特征重要性已保存到 lasso_feature_importance.csv")
    else:
